...
int main(int argc, char *argv[]) {
  ...
  if(sprintf(message, "Connecting message...") < 0) {
    error("creating connecting message");
  }
  if(sendto(fd, message, strlen(message), 0, (struct sockaddr*)&addr, addr_size) == -1) {
    error("sending connecting message");
  }
